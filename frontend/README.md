...
### 2. **Удалите тома (volumes)**
Если ваша база данных использует Docker-тома (volumes), их также нужно удалить, чтобы очистить данные. Выполните:

```bash
docker-compose down -v
```

Флаг `-v` удаляет все тома, связанные с контейнерами.

---

### 3. **Пересоздайте контейнеры и запустите миграции**
...
После запуска контейнеров выполните миграции. Способ выполнения миграций зависит от вашего приложения. Например:


### Важно:
- Убедитесь, что вы действительно хотите удалить данные базы данных, так как этот процесс необратим.
- Если вы используете Docker-тома, данные будут полностью удалены при использовании флага `-v`.
- Если данные хранятся на хосте, убедитесь, что вы удаляете правильную директорию.
